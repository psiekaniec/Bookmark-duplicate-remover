...
def main(filename):
    ...
    lists_of_bookmarks = [dict_with_key for dict_with_key in content["children"] if "children" in dict_with_key]
    result = []
    for l in lists_of_bookmarks:
        try:
            bookmarks = sorted(l["children"], key=lambda b: b["uri"])
            for i in range(len(bookmarks)):
                if not bookmarks[i-1]["uri"] == bookmarks[i]["uri"] or len(bookmarks) == 1:
